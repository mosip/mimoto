...
kubectl create ns $NS || true  # Continue if namespace already exists
...
function installing_mimoto() {
  ...
  kubectl -n config-server get deploy -o name | xargs -n1 -t kubectl -n config-server1 rollout status
  ...
  echo "Please enter the Google Client ID "
  read -p "" clientId
  ...
  echo "Please enter the Google Secret Key"
  read -p "" secretKey
  ...
  helm -n $NS install mimoto mosip/mimoto --version $CHART_VERSION $ENABLE_INSECURE \
