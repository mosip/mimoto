...
public class WalletsController {
     ...
     */
    @Operation(summary = "Create a new wallet", description = "This API is secured using session-based authentication. The session ID is extracted from the Cookie header and used to retrieve session details from Redis for authentication. The user ID is obtained from the session and used with the provided wallet name and PIN to create a new wallet in the database. If successful, a unique identifier for the wallet is returned; otherwise, an appropriate error response is provided.", operationId = "createWallet", security = @SecurityRequirement(name = "SessionAuth"), requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(required = true, description = "Details required to create a wallet", content = @Content(mediaType = "application/json", schema = @Schema(implementation = CreateWalletRequestDto.class), examples = @ExampleObject(name = "Valid Request", value = "{ \"walletPin\": \"1234\", \"confirmWalletPin\": \"1234\", \"walletName\": \"MyWallet\" }"))))
    ...
    @ApiResponse(responseCode = "400", description = "Invalid wallet creation request", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ErrorDTO.class), examples = {
            @ExampleObject(name = "User ID is not present in session", value = "{\"errorCode\": \"invalid_request\", \"errorMessage\": \"User ID not found in session\"}"),
            ...
            @ExampleObject(name = "Wallet Pin and confirm Pin Mismatch", value = "{\"errorCode\": \"invalid_request\", \"errorMessage\": \"Entered Passcode and Confirm Passcode do not match\"}")}))
    ...
    public ResponseEntity<String> createWallet(
        ...
        String walletId = walletService.createWallet(userId, wallet.getWalletName(), wallet.getWalletPin(), wallet.getConfirmWalletPin());
    ...
    @ApiResponse(responseCode = "400", description = "Invalid Wallets fetching request", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ErrorDTO.class), examples = {@ExampleObject(name = "User ID is not present in session", value = "{\"errorCode\": \"invalid_request\", \"errorMessage\": \"User ID not found in session\"}"), @ExampleObject(name = "Wallet not found", value = "{\"errorCode\": \"invalid_request\", \"errorMessage\": \"Wallet not found\"}"), @ExampleObject(name = "Invalid User ID", value = "{\"errorCode\": \"invalid_request\", \"errorMessage\": \"User ID cannot be null or empty\"}"), @ExampleObject(name = "Invalid Wallet Pin", value = "{\"errorCode\": \"invalid_request\", \"errorMessage\": \"Passcode must be numeric with 6 digits\"}")}))
     ...
     */
    @Operation(summary = SwaggerLiteralConstants.WALLETS_DELETE_SUMMARY, description = SwaggerLiteralConstants.WALLETS_DELETE_DESCRIPTION, operationId = "deleteWallet", security = @SecurityRequirement(name = "SessionAuth"), parameters = {@Parameter(name = "walletId", in = ParameterIn.PATH, required = true, description = "Unique identifier of the wallet to be deleted", schema = @Schema(type = "string"))})
