...
function installing_onboarder() {
  ...
  if [[ $yn = "Y" || $yn = "y" ]]; then
    ...
    echo "n: if you don't have a public domain and a valid SSL certificate. It will add a self-signed cert in onboarder docker (recommended for testing only)."
    read -p "" flag
    ...
    echo "Installing onboarder in namespace: $NS"
    ...
    echo "Updating Mimoto wallet binding partner API key and OIDC Partner Client ID"
    ...
    echo "Reports are moved to S3 under onboarder bucket (if applicable)"
    return 0
  fi
}
